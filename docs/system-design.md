...
Project A 是一个面向大学课程设计的轻量版特定领域智能问答系统。系统围绕企业制度文档构建本地知识库，使用检索增强生成思路回答问题，并提供引用来源用于回溯。
...
- 向量模块：为文本块生成向量并写入 FAISS
- 问答模块：检索 Top 5 证据片段，拼接 Prompt，请求 Ollama 生成答案
...
- `data/uploads/`：原始上传文件
- `data/index/faiss.index` 或 `data/index/embeddings.npy`
- `data/index/metadata.json`：向量索引与 chunk 映射关系
